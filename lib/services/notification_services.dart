...
class NotifyHelper {
  ...
  initializeNotification() async {
    ...
    const AndroidInitializationSettings initializationSettingsAndroid =
        AndroidInitializationSettings('apppic');
  ...
  scheduledNotification(int hour, int minutes, Task task) async {
    debugPrint('check if the scheduledNotification working');
...
print('now= $now');
    ...
    scheduledDate = buildRemind(remind, scheduledDate);

    if (scheduledDate.isBefore(now)) {
      if(repeat=='Daily'){
        scheduledDate = tz.TZDateTime(tz.local, now.year, FormattedDate.month, (FormattedDate.day)+1, hour, minutes);
      }
      if(repeat=='Weekly'){
        scheduledDate = tz.TZDateTime(tz.local, now.year, FormattedDate.month, (FormattedDate.day)+7, hour, minutes);
      }
      if(repeat=='Monthly'){
  ...
  tz.TZDateTime buildRemind(int remind, tz.TZDateTime scheduledDate) {
    if(remind==5){
